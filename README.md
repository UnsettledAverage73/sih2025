...
*   **Language**: TypeScript (Frontend) & Rust (Tauri Backend) & Go (Wiping Logic)
*   **Styling**: TailwindCSS (v3.x) & shadcn/ui
*   **Backend Logic (Go)**: Secure wiping logic implemented in `wipe_tool` Go backend, including device scanning.
...
*   [Go](https://go.dev/doc/install) (latest stable version recommended)
...
2.  **Install frontend dependencies:**
    Navigate to the `frontend/ui` directory and install the Node.js dependencies:
    ```bash
    cd frontend/ui
    npm install
...
3.  **Build the Go backend `wipe_tool`:**
    Navigate to the `backend` directory and run the Makefile command to build the wiping tool:
    ```bash
    cd backend
    make build
    cd .. # Go back to the project root
...
```
/sih2025
├── frontend/             # Frontend application (Next.js, React, TypeScript, Tauri config)
│   └── ui/               # Next.js UI source
│       ├── public/
│       ├── src/
...
│       ├── src-tauri/        # Tauri Rust backend source code (part of frontend build process)
│       │   ├── src/
...
│       ├── tailwind.config.ts
│       └── tsconfig.json
├── backend/              # Go backend services and build artifacts
│   ├── build/            # Compiled Go backend binaries
│   ├── cmd/              # Go backend commands
│   │   └── wipe/         # Wipe tool command
│   ├── internals/        # Go internal packages
│   │   ├── scanner/      # Device scanning logic
│   │   └── wipe/         # Core wiping logic (OS-specific implementations)
